...
null = lm(Item_Outlet_Sales~1,data=train)
full = lm(Item_Outlet_Sales~.,data=train)
step(null,scope = list(lower=null,upper=full),direction = 'forward')
m1 = lm(formula = Item_Outlet_Sales ~ Item_MRP + Outlet_Identifier, 
        data = train)
...
head(sub1)
write.csv(sub1,'BigMart3-1.csv',row.names = F)
