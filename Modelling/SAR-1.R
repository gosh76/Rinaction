...
sum(is.na(df))
ha = aggregate(df[,c('zipcode','price')],by=list(df$zipcode),FUN='mean')
dim(ha)
ha
zip1 = ha[ha$price==max(ha$price),'zipcode']
zip1#98039
price1 = max(ha$price)
price1
head(df)
...
#rmse difference
diff1 = rmse1 - rmse2
diff1#54473.43 - model m2 using advanced_features is better.
# backward elimination from advanced_features
# m2 model is trained with advanced_features
attach(train2)
#fm1 = fastbw(lrm(price~bedrooms+bathrooms+sqft_living+sqft_lot+floors+zipcode+condition       
...
difference2#-51846.87
#m1 using my_features performs better 
