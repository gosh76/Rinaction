...
p1 = c(0,target[1:nrow(train)-1])
train$pre = p1
train$Count = target
str(train)
str(test)
library(randomForest)
rf1 = randomForest(Count~.,data=train)
test$pre = 0
head(test)
pred = predict(rf1,newdata = test,type='class')
pred[1:5]
p2 = c(0,pred[1:nrow(test)-1])
test$pre=p2
pred1 = predict(rf1,newdata = test,type='class')
sub1 = data.frame(ID=test$ID)
head(sub1)
sub1$Count = pred1
head(sub1)
write.csv(sub1,'TimeSeries4.csv',row.names = F)
